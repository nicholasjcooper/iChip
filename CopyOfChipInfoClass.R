...
#' @slot seqnames Rle, containing chromosomes for each range, see GRanges.
#' @slot ranges IRanges, containing genomic start and end, see GRanges.
#' @slot strand Rle, containing plus or minus coding for forward or reverse strand, see GRanges.
#' @slot elementMetaData DataFrame, see GRanges, but with specific column names:  A1, A2, QCcode and rs.id.
#' @slot seqinfo Seqinfo, containing chromosome listing, see GRanges.
#' @slot metadata list, for extra information, mainly for GRanges compatibility.
#' @slot chip Character, containing user description of the chip, e.g, 'immunoChip'.
#' @slot build Character, annotation version, e.g, hg18, hg19, hg38, etc.
# @name ChipInfo-class
...
setClass("ChipInfo",
         contains="GRanges",
         representation=list(seqnames="Rle",
                          ranges="IRanges",
                          strand="Rle",
